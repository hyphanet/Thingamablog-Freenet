...
public class DefaultMIMETypes {
		...
		if(extensions != null) {
			for(int i=0;i<extensions.length;i++) {
				...
				if(DefaultMIMETypes.mimeTypesByExtension.containsKey(ext)) {
					...
					logger.log(Level.INFO, "Extension "+ext+" assigned to "+DefaultMIMETypes.byNumber(s.shortValue())+" in preference to "+number+":"+type);
				} else {
	...
	public synchronized static String guessMIMEType(final String arg) {
		if (arg == null) {
                        // Logger can't be used here (static method)
			System.out.println("WARNING : Unable to guess mime type for file 'null' ");
			return null;
		}
