...
{    
	...
	{		
		...
		//init the templates
		mainTemplate = new TBTemplate(new File(tmplDir, "main.template"), "Front Page");
		templates.add(mainTemplate);
		archiveTemplate = new TBTemplate(new File(tmplDir, "archive.template"), "Archive");
		templates.add(archiveTemplate);
		categoryTemplate = new TBTemplate(new File(tmplDir, "category.template"), "Category");
		templates.add(categoryTemplate);
		entryTemplate = new TBTemplate(new File(tmplDir, "entry.template"), "Entry Pages");
		templates.add(entryTemplate);
		arcIndexTemplate = new TBTemplate(new File(tmplDir, "index.template"), "Archive Index");
		templates.add(arcIndexTemplate);
		feedTemplate = new TBTemplate(new File(tmplDir, "feed.template"), "Feed");
	...
	public void deleteAll() throws BackendException
	{
		...
		net.sf.thingamablog.Utils.deleteDir(outputDir);
		net.sf.thingamablog.Utils.deleteDir(tmplDir);
		if(webFilesDirectory.getParent().equals(homeDir.getAbsolutePath()))
			net.sf.thingamablog.Utils.deleteDir(webFilesDirectory);
