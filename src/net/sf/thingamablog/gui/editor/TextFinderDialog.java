...
public class TextFinderDialog extends JDialog
{
    ...
    private static final String TITLE = Messages.getString("TextFinderDialog.Find_and_Replace"); //$NON-NLS-1$
	...
	private void init(JTextComponent tc, int index)
	{
		...
		pf.add(new JLabel(Messages.getString("TextFinderDialog.Find_what")));
        
		txtFind1 = new JTextField();
		docFind = txtFind1.getDocument();
		pf.add(txtFind1);
		pc1.add(pf, BorderLayout.CENTER);
		popupManager.addJTextComponent(txtFind1);

		JPanel po = new JPanel(new GridLayout(2, 2, 8, 2));
		po.setBorder(new TitledBorder(new EtchedBorder(), Messages.getString("TextFinderDialog.Options")));

		JCheckBox chkWord = new JCheckBox(Messages.getString("TextFinderDialog.Whole_words_only"));
		chkWord.setMnemonic('w');
		modelWord = chkWord.getModel();
		po.add(chkWord);

		ButtonGroup bg = new ButtonGroup();
		JRadioButton rdUp = new JRadioButton(Messages.getString("TextFinderDialog.Search_up"));
		rdUp.setMnemonic('u');
		modelUp = rdUp.getModel();
		bg.add(rdUp);
		po.add(rdUp);

		JCheckBox chkCase = new JCheckBox(Messages.getString("TextFinderDialog.Match_case"));
		chkCase.setMnemonic('c');
		modelCase = chkCase.getModel();
		po.add(chkCase);

		JRadioButton rdDown = new JRadioButton(Messages.getString("TextFinderDialog.Search_down"), true);
		...
		JButton btFind = new JButton(Messages.getString("TextFinderDialog.Find_Next"));
		...
		JButton btClose = new JButton(Messages.getString("TextFinderDialog.Close"));
		...
		tb.addTab(Messages.getString("TextFinderDialog.Find"), p1);
		...
		pc.add(new JLabel(Messages.getString("TextFinderDialog.Find_what")));
		txtFind2 = new JTextField();
		txtFind2.setDocument(docFind);
		pc.add(txtFind2);
		popupManager.addJTextComponent(txtFind2);

		pc.add(new JLabel(Messages.getString("TextFinderDialog.Replace")));
		JTextField txtReplace = new JTextField();
		docReplace = txtReplace.getDocument();
		pc.add(txtReplace);
		pc2.add(pc, BorderLayout.CENTER);
		popupManager.addJTextComponent(txtReplace);

		po = new JPanel(new GridLayout(2, 2, 8, 2));
		po.setBorder(new TitledBorder(new EtchedBorder(), Messages.getString("TextFinderDialog.Options")));

		chkWord = new JCheckBox(Messages.getString("TextFinderDialog.Whole_words_only"));
		chkWord.setMnemonic('w');
		chkWord.setModel(modelWord);
		po.add(chkWord);

		bg = new ButtonGroup();
		rdUp = new JRadioButton(Messages.getString("TextFinderDialog.Search_up"));
		rdUp.setMnemonic('u');
		rdUp.setModel(modelUp);
		bg.add(rdUp);
		po.add(rdUp);

		chkCase = new JCheckBox(Messages.getString("TextFinderDialog.Match_case"));
		chkCase.setMnemonic('c');
		chkCase.setModel(modelCase);
		po.add(chkCase);

		rdDown = new JRadioButton(Messages.getString("TextFinderDialog.Search_down"), true);
		rdDown.setMnemonic('d');
		...
		JButton btReplace = new JButton(Messages.getString("TextFinderDialog.Replace"));
		...
		{ 
			public void actionPerformed(ActionEvent e) 
			{
				...
				JOptionPane.showMessageDialog(owner, 
					counter+ " " + Messages.getString("TextFinderDialog.replacements_prompt"), "Info",
					JOptionPane.INFORMATION_MESSAGE);
			}
		};
		JButton btReplaceAll = new JButton(Messages.getString("TextFinderDialog.Replace_All"));
		btReplaceAll.addActionListener(replaceAllAction);
		btReplaceAll.setMnemonic('a');
		p.add(btReplaceAll);

		btClose = new JButton(Messages.getString("TextFinderDialog.Close"));
		btClose.addActionListener(closeAction);
		btClose.setDefaultCapable(true);
		p.add(btClose);
		p02.add(p);
		...
		tb.addTab(Messages.getString("TextFinderDialog.Replace"), p2);
	...
	public int findNext(boolean doReplace, boolean showWarnings)
	{
		...
		if(key.length()==0) 
		{
			warning(Messages.getString("TextFinderDialog.no_target_prompt"));
			return -1;
		}
		...
		if(modelWord.isSelected()) 
		{
			for(int k=0; k<WORD_SEPARATORS.length; k++) 
			{
				...
				{
					warning(Messages.getString("TextFinderDialog.illegal_character_prompt") +
						" \'"+WORD_SEPARATORS[k]+"\'");
					return -1;
				}
			}
		...
		while(true)
		{
			...
			{
				if(showWarnings)
					warning(Messages.getString("TextFinderDialog.Text_not_found"));
				return 0;
			}
